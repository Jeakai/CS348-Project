...
SELECT * FROM favourites WHERE uid = 1 AND pid = 126;

-- unfavouriting player     feature 5b
DELETE FROM favourites WHERE uid = 1 AND pid = 126;
...
WITH combined_pts (pid, tid, season, pts) AS (
    ...
    FROM members
    WHERE tid IN (SELECT tid FROM teams WHERE abbreviation = "LAL")
